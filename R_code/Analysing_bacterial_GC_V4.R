...
setwd("C:/Users/Mole/Documents/Uni work/UNI WORK/Year 4/The project/learningR_2021_1styrs/Bacteria_CSV")

if (file.exists("GCvStopusage.xls")) {l
file.remove("GCvStopusage.xls")
}
...
#Everthing on the same graph

pdf("Stop_Codons_same.pdf")

aa.perc <- unlist(data1[20])

aa <- colnames(data1)[20]
...
plot(GC3.inR, aa.perc, xlab="GC3%", ylab = "% use of stop codon", col = "blue", cex=0.5, pch=23, main = plot.title, xlim = c(0,100), ylim =c(0,100))
...
aa.perc <- unlist(data1[23])

aa <- colnames(data1)[23]
...
plot(GC3.inR, aa.perc, xlab="", ylab = "", col = "red", cex=0.5, pch=15, main = "", xlim = c(0,100), ylim =c(0,100))


abline(lm(aa.perc~GC3.inR), col = "black", lty = 2)
Rsquare <- lm(aa.perc~GC3.inR)
...
aa.perc <- unlist(data1[26])

aa <- colnames(data1)[26]
...
plot(GC3.inR, aa.perc, xlab="", ylab = "", col = "gray0", cex=0.5, pch=19, main = "", xlim = c(0,100), ylim =c(0,100))


abline(lm(aa.perc~GC3.inR), col = "black", lty = 3)
Rsquare <- lm(aa.perc~GC3.inR)
...
aa.perc <- unlist(data1[29])

aa <- colnames(data1)[29]
